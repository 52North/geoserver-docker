...
function download_extension() {
  ...
  if [ -e "$DOWNLOAD_FILE" ]; then
    ...
    if wget --quiet --method=HEAD "${URL}"; then
...
# Download and install stable plugins only if INSTALL_EXTENSIONS is true
if [ "$INSTALL_EXTENSIONS" = "true" ]; then
  ...
  for EXTENSION in $(echo "${COMMUNITY_EXTENSIONS}" | tr ',' ' '); do
    URL="${COMMUNITY_PLUGIN_URL}/geoserver-${GEOSERVER_VERSION}-SNAPSHOT-${EXTENSION}-plugin.zip"
    download_extension "${URL}" "${EXTENSION}"
  done
